...
const Questionnaire = () => {
  ...
  const handleInputChange = (event) => {
    ...
    const questionText = quiz3.questions[currentQuestionIndex].question;
  ...
  useEffect(() => {
    const currentQuestionText = quiz3.questions[currentQuestionIndex].question;
  ...
  const handleNextQuestion = () => {
    ...
    if (isCurrentQuestionAnswered) {
      if (currentQuestionIndex < quiz3.questions.length - 1) {
  ...
  const currentQuestion = quiz3.questions[currentQuestionIndex];
  const isLastQuestion = currentQuestionIndex === quiz3.questions.length - 1;
  ...
  const handleGeneratePDF = () => {
    ...
    // Redirect to /Trial using Next.js router after the PDF is generated
    router.push("/Trial");
  };
  return (
      ...
      {!isLastQuestion ? (
        <div className="w-11/12 md:w-8/12 h-[436px] rounded-[51px] shadow-2xl shadow-green-400">
          <div className="mt-6 pl-6 w-11/12 md:w-9/12 items-start justify-center flex flex-col">
            <h1 className="font-bold text-2xl">Generate diet plan for Fertility</h1>
        ...
        <div className="m-auto w-11/12">
          {result ? (
            <>
              ...
              <h2 className="text-xl font-bold mb-3">Your Fertility Diet Plan Is:</h2>
              <div
